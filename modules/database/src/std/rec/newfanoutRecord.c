...
epicsExportAddress(rset,newfanoutRSET);


static void checkAlarms(newfanoutRecord *);
static void monitor(newfanoutRecord *);
static void push_values(newfanoutRecord *);

//#define OUT_ARG_MAX 8
#define OUT_ARG_MAX 768

static long init_record(struct dbCommon *pcommon, int pass)
{
    struct newfanoutRecord *prec = (struct newfanoutRecord *)pcommon;
    if (pass==0)
        return 0;

    recGblInitConstantLink(&prec->sell, DBF_USHORT, &prec->seln);
    printf("init_record,  newfanoutRecord");
...

#define indexof(field) newfanoutRecord##field

static long get_units(DBADDR *paddr,char *units)
{
    newfanoutRecord *prec=(newfanoutRecord *)paddr->precord;
...

static void checkAlarms(newfanoutRecord *prec)
{
...

static void monitor(newfanoutRecord *prec)
{
...
static void push_values(newfanoutRecord *prec)
{
    ...
    switch (prec->selm){
    case (newfanoutSELM_All):
        for(i=0, plink=&(prec->out000); i<OUT_ARG_MAX; i++, plink++) {
                status=dbPutLink(plink,DBR_DOUBLE,&(prec->val),1);
                if(status) recGblSetSevr(prec,LINK_ALARM,MAJOR_ALARM);
        }
        break;
    case (newfanoutSELM_Specified):
        ...
        if(status) recGblSetSevr(prec,LINK_ALARM,MAJOR_ALARM);
        break;
    case (newfanoutSELM_Mask):
        if(prec->seln==0) break;
        for(i=0, plink=&(prec->out000), state=prec->seln;
        i<OUT_ARG_MAX;
        i++, plink++, state>>=1) {
            if(state&1) {
                status=dbPutLink(plink,DBR_DOUBLE,&(prec->val),1);
